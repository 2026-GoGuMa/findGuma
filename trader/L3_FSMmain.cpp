...
static void L3_action_sendPriceCnf(void) {     // 가격 CNF 패킷 빌드 후 전송
...
static void L3_action_sendLocCnf(void) {        // 위치 CNF 패킷 빌드 후 전송 
  uint8_t size = L3_msg_buildCnf(sdu, myId, coordId, seq_num++,
                                  0, rcvd_avg_loc);
  L3_LLI_dataReqFunc(sdu, size, coordId);
...
static void L3_action_reset(uint8_t success) {   // 변수 초기화 + 결과 출력
  rcvd_avg_price     = 0;
  rcvd_avg_loc       = 0;
...
void L3_FSMrun(void) {
  ...
  if (L3_event_checkEventFlag(L3_event_msgRcvd)) {
    L3_parse_msg();     // PDU 분류 → 세부 이벤트 플래그 세팅
  }
